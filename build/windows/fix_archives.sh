...
find dist -regex ".*_dirty\.zip" | while read zip_dirty; do
  ...
  mv ${ZIP_CONTENT_PATH}/${INTEGRATION}-win-definition.yml ${ZIP_CONTENT_PATH}/${INTEGRATION}-definition.yml
  mv ${ZIP_CONTENT_PATH}/${INTEGRATION}-definition.yml "${AGENT_DIR_IN_ZIP_PATH}"
